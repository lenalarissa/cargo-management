...
public class UpdateCargoListener implements Listener<StorageLocationEvent> {
    private final ManagementFacade managementFacade;
    private Handler<ActionDLEvent> writeLogHandler;
    private Handler<NetEvent> serverHandler;

    public UpdateCargoListener(ManagementFacade managementFacade) {
    ...
    public void onEvent(StorageLocationEvent event) {
        if (writeLogHandler != null) {
            writeLogHandler.handle(new ActionDLEvent(this, ActionDL.UPDATECARGO));
        }
        CargoStorage cargoStorage = managementFacade.getCargoStorage();
        boolean result = cargoStorage.updateCargo(event.getStorageLocation());
